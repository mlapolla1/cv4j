...
public class GradientFilter {
     ...
     * Sobel operator x
     */
    private final static int[][] SOBEL_X = new int[][]{{-1, 0, 1}, {-2, 0, 2}, {-1, 0, 1}};

    /**
     * Sobel operator x
     */
    private final static int[][] SOBEL_Y = new int[][]{{-1, -2, -1}, {0, 0, 0}, {1, 2, 1}};
    ...
    public int[] gradient(ByteProcessor src) {
        ...
        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width; col++) {
                ...
                max = Math.max(Tools.clamp(SafeCasting.safeDoubleToInt(mred)), max);
                min = Math.min(Tools.clamp(SafeCasting.safeDoubleToInt(mred)), min);
    ...
    private void setOutPixels(int pixelDirection, int index, double mred, int[] outPixels) {
        switch (pixelDirection) {
            case X_DIRECTION:
                outPixels[index] = Tools.clamp(SafeCasting.safeDoubleToInt(yred));
                break;
            case Y_DIRECTION:
                outPixels[index] = Tools.clamp(SafeCasting.safeDoubleToInt(xred));
                break;
            default:
                // XY_DIRECTION
                outPixels[index] = Tools.clamp(SafeCasting.safeDoubleToInt(mred));
                break;
